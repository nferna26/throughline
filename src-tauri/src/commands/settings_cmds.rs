...
/// DATA-004: the canonical targets (`Books/`, `Sessions/`) are probed too when
/// they exist — a `Books` that is a stray file, or an unwritable one, breaks
/// every later note export, and probing only the root missed that. A MISSING
/// subfolder stays untouched (it is created on the first export; the launch
/// probe never creates anything — CORE-1019).
fn export_write_probe(root: &std::path::Path) -> Result<(), String> {
    ...
    for sub in ["Books", "Sessions"] {
        ...
        if !dir.is_dir() {
            return Err(format!(
                "Something named {sub} is in the way inside the export folder — it needs to be a folder. Move or rename that file, or pick a different export folder."
            ));
        }
        let probe = dir.join(".throughline-write-test");
        std::fs::write(&probe, b"ok").map_err(|e| {
            format!("Throughline can't save Markdown into the {sub} folder here ({e}).")
        })?;
        let _ = std::fs::remove_file(&probe);
    }
    Ok(())
...
mod tests {
    ...
    /// DATA-004: the launch probe must flag a `Books` that is a stray FILE —
    /// every note export fails against it — while never CREATING a missing
    /// Books/Sessions (they appear on first export).
    #[test]
    fn launch_check_flags_books_as_file_but_never_creates_subdirs() {
        ...
        // A bare existing root: writable, and the probe must not plant subdirs.
        let v = check_export_root(&dir);
        assert_eq!(v["writable"], true);
        assert!(
            !dir.join("Books").exists() && !dir.join("Sessions").exists(),
            "the launch probe must not create Books/Sessions"
        );

        // Now a stray FILE named Books: the probe must say so.
        std::fs::write(dir.join("Books"), b"in the way").unwrap();
        let v = check_export_root(&dir);
        assert_eq!(v["writable"], false, "Books-as-file must flag unwritable");
        let msg = v["message"].as_str().unwrap_or_default();
        assert!(
            msg.contains("Books"),
            "message names the obstruction: {msg}"
        );
        std::fs::remove_dir_all(&dir).ok();
    }

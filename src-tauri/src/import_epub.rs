...
pub fn import_epub(src_path: &Path) -> Result<ImportResult> {
    ...
    if looks_drm_protected(src_path).unwrap_or(false) {
        return Err(anyhow!(
            "this EPUB looks DRM-protected (encryption.xml is present). Throughline refuses to process DRM-protected files. Please use a DRM-free EPUB."
        ));
    }
    ...
    for (i, (entry, ex)) in sections_input.iter().zip(&extracts).enumerate() {
        ...
        sections.push(BookSection {
            ...
            estimated_units: if ex.char_count > 0 { Some(ex.char_count as i64) } else { None },
            sort_order: i as i64,
...
fn build_spine_entries<R: std::io::Read + std::io::Seek>(
    ...
    for (i, item) in doc.spine.iter().enumerate() {
        ...
        entries.push(SpineEntry { idref: item.idref.clone(), label, href: href_no_frag, assignable });
    }
...
fn extract_sections<R: std::io::Read + std::io::Seek>(
    ...
    for entry in entries {
        ...
        out.push(SectionExtract { start, end, char_count, ranges: extracted.ranges });
    }
    (body, out)
}
...
fn write_epub_text_artifacts(
    ...
    set_readonly(&txt);
    fs::write(book_dir.join("structure.json"), serde_json::to_string(structure)?)?;
    fs::write(
        book_dir.join("body_offsets.json"),
        serde_json::to_string(&serde_json::json!({ "body_start": 0usize, "body_end": body.len() }))?,
    )?;
    Ok(())
}
...
pub fn ensure_epub_text(conn: &rusqlite::Connection, book_id: &str) -> Result<bool> {
    ...
    let ids: Vec<String> = {
        let mut stmt =
            conn.prepare("SELECT id FROM book_sections WHERE book_id = ?1 ORDER BY sort_order ASC")?;
...
fn decode_one_entity(ent: &str) -> Option<String> {
    ...
    let c = match ent {
        ...
        "ccedil" => 'ç', "ocirc" => 'ô', "ouml" => 'ö', "uuml" => 'ü', "auml" => 'ä',
        "iuml" => 'ï', "icirc" => 'î', "ugrave" => 'ù', "ntilde" => 'ñ', "szlig" => 'ß',
        "oelig" => '\u{0153}', "aelig" => '\u{00E6}', "euro" => '\u{20AC}', "pound" => '\u{00A3}',
        _ => return None,
    };
    Some(c.to_string())
}
...
fn is_code_class(class: &str) -> bool {
    ...
    c.contains("code") || c.contains("programlisting") || c.contains("listing") || c.contains("sourcecode")
}

/// A code sub-element that is a line-number / prefix gutter, not code text.
fn is_gutter_class(class: &str) -> bool {
    let c = class.to_ascii_lowercase();
    c.contains("codeinfo") || c.contains("codeprefix") || c.contains("lineno")
        || c.contains("linenum") || c.contains("gutter")
}
...
fn normalize_code(buf: &str) -> String {
    ...
    let last = lines.iter().rposition(|l| !l.is_empty()).map(|i| i + 1).unwrap_or(0);
    if first >= last {
        return String::new();
    }
...
fn class_of(raw: &str) -> String {
    ...
    let Some(p) = lower.find("class=") else { return String::new() };
    let rest = &raw[p + 6..];
...
impl Extractor {
    fn new() -> Self {
        Extractor { at_para_start: true, ..Default::default() }
    }
    ...
    fn close_code(&mut self) {
        ...
        if !code.is_empty() {
            ...
            let end = self.u16;
            self.ranges.push(StyleRange { kind: "pre".into(), start, end });
            self.at_para_start = false;
            self.para_break();
        }
    }
    ...
    fn close_inline(&mut self, kind: &str) {
        if let Some(pos) = self.inline_stack.iter().rposition(|(k, _)| k == kind) {
            ...
            if self.u16 > start {
                self.ranges.push(StyleRange { kind: kind.to_string(), start, end: self.u16 });
            }
        }
    }
    ...
    fn close_block(&mut self, track: bool) {
        if track {
            if let Some((kind, start)) = self.block_stack.pop() {
                if self.u16 > start {
                    self.ranges.push(StyleRange { kind, start, end: self.u16 });
                }
            }
        }
        self.para_break();
    ...
    fn handle_tag(&mut self, name: &str, class: &str, is_close: bool, is_selfclose: bool) {
        ...
        if self.code.is_some() {
            ...
            if name == self.code_closer {
                if is_close {
                    ...
                    if self.code_depth == 0 { self.close_code() } else { self.code_newline() }
        ...
        match name {
            ...
            "h1" | "h2" | "h3" | "h4" | "h5" | "h6" | "blockquote" => {
                if is_close { self.close_block(true) } else { self.open_block(name, true) }
            }
            "p" | "div" | "li" | "ul" | "ol" | "dd" | "dt" | "section" | "article" | "header"
            | "footer" | "nav" | "aside" | "main" | "figure" | "figcaption" | "table" | "tr"
            | "td" | "th" | "caption" => {
                if is_close { self.close_block(false) } else { self.open_block(name, false) }
            }
            "hr" => self.para_break(),
            "br" => self.pending_space = true,
            "strong" | "b" => {
                if is_close { self.close_inline("strong") } else if !is_selfclose { self.open_inline("strong") }
            }
            "em" | "i" => {
                if is_close { self.close_inline("em") } else if !is_selfclose { self.open_inline("em") }
            }
    ...
    fn finish(mut self) -> ExtractedSection {
        ...
        let ranges: Vec<StyleRange> = self
            ...
            .filter_map(|mut r| {
                ...
                if r.end > r.start { Some(r) } else { None }
            })
            .collect();
        ExtractedSection { text: self.out, ranges }
    }
}
...
pub fn extract_section(html: &str) -> ExtractedSection {
    ...
    let find_ci = |i: usize, pat_lower: &str| -> Option<usize> {
        ...
        if p.is_empty() || i >= n { return None; }
        (i..=n.saturating_sub(p.len()))
            .find(|&j| chars[j..j + p.len()].iter().zip(&p).all(|(c, pc)| c.to_ascii_lowercase() == *pc))
    };
    let mut i = 0;
    while i < n {
        if chars[i] == '<' {
            ...
            if !name.is_empty() {
                let class = if raw.to_ascii_lowercase().contains("class=") { class_of(&raw) } else { String::new() };
...
mod tests {
    ...
    fn collapses_whitespace_and_strips_tag_attributes() {
        assert_eq!(text_of("<p class=\"x\">Hello\n\n   world</p>"), "Hello world");
    }
    ...
    fn decodes_html_named_and_numeric_entities() {
        assert_eq!(text_of("<p>Tom &amp; Jerry &mdash; &#8220;hi&#x2019;&nbsp;there</p>"), "Tom & Jerry — “hi’ there");
    ...
    fn records_heading_range_over_its_paragraph() {
        ...
        assert_eq!((h.start, h.end), (0, "Chapter One".encode_utf16().count() as u32));
    }
    ...
    fn maps_b_to_strong_and_i_to_em() {
        ...
        assert!(s.ranges.iter().any(|r| r.kind == "strong" && r.start == 0 && r.end == 4));
    ...
    fn pre_block_preserves_indentation_and_newlines_as_one_code_block() {
        ...
        let code = s.ranges.iter().find(|r| r.kind == "pre").expect("pre range");
        let utf16: Vec<u16> = s.text.encode_utf16().collect();
    ...
    fn table_code_listing_becomes_single_spaced_lines_gutter_stripped() {
        ...
        assert!(s.ranges.iter().any(|r| r.kind == "pre"), "table code → a pre range");
        assert!(!s.ranges.iter().any(|r| r.kind == "strong"), "code keywords must not become bold");
    }
    ...
    fn estimated_units_track_real_prose_length() {
        ...
        assert_eq!(n, "The quick brown fox jumps over the lazy dog.".chars().count());
